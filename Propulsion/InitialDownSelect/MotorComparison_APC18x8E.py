...
#IMPORT PROP
from Propellers.APC_18x8E import Prop
#IMPORT Battery
...
Propulsion += [ACPropulsion(Prop,AXI_5325_18)] ; legend += ['AXI_5325_18']                                       # Selected - use!
...
Propulsion += [ACPropulsion(Prop,Hacker_A50_16L)] ; legend += ['Hacker_A50_16L']                               # Selected - use!
...
Propulsion += [ACPropulsion(Prop,Hacker_A60_5S)] ; legend += ['Hacker_A60_5S']                                   # Selected - use!
...
Propulsion += [ACPropulsion(Prop,Hacker_A60_7XS)] ; legend += ['Hacker_A60_7XS']                                 # Selected - use!
# 
# # Propulsion += [ACPropulsion(Prop,Hyperion_ZS4025_14)] ; legend += ['Hyperion_ZS4025_14']
Propulsion += [ACPropulsion(Prop,Hyperion_ZS4025_16)] ; legend += ['Hyperion_ZS4025_16']                           # Selected - use!
# # Propulsion += [ACPropulsion(Prop,Hyperion_ZS4035_10)] ; legend += ['Hyperion_ZS4035_10']
Propulsion += [ACPropulsion(Prop,Hyperion_ZS4035_12)] ; legend += ['Hyperion_ZS4035_12']                           # Selected - use!
Propulsion += [ACPropulsion(Prop,Hyperion_ZS4045_10)] ; legend += ['Hyperion_ZS4045_10']                           # Selected - use!
...
for i in xrange(len(Propulsion)): # This line changed to print symbols on plots -- 10/1/14 Bo Landess
    pp = Propulsion[i]
    fmt = linefmt[i]    # This line changed to print symbols on plots -- 10/1/14 Bo Landess
    ...
    pyl.figure(2)
    pyl.subplot(221)
    pyl.plot(V/(FT/SEC), Ib,fmt) # Added fmt LineSpec 10/1/14 BL
    pyl.xlabel('V (ft/s)'); pyl.ylabel('Current (A)') 
    pyl.subplot(222)
    pyl.plot(V/(FT/SEC), pp.Engine.Duration(N=Nm)/MIN,fmt) # Added fmt LineSpec 10/1/14 BL
    pyl.xlabel('V (ft/s)'); pyl.ylabel('Duration (min)')

    pyl.subplot(223)
    pyl.plot(V/(FT/SEC), pp.Engine.Pin(Nm)/W,fmt)# Added fmt LineSpec 10/1/14 BL
    pyl.xlabel('V (ft/s)'); pyl.ylabel('Power In (watt)') 
    pyl.subplot(224)
    pyl.plot(V/(FT/SEC), pp.Engine.Efficiency(Nm), fmt)# Added fmt LineSpec 10/1/14 BL
...
pyl.show()
print 'End' # Added to properly terminate script